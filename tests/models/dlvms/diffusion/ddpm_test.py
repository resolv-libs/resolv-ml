...
class DDPMTest(unittest.TestCase):
    ...
    def get_ddpm_model(self) -> DDPM:
        model = DDPM(
            ...
            denoising_layer=keras.layers.Lambda(lambda x: x[0]),
            timesteps=1000
        )
    ...
    def _test_model(self, ddpm_model: DDPM, output_name: str):
        ...
        predicted_sequences, predicted_noise, noise = ddpm_model.predict(
        ...
        predicted_sequences, noisy_sequences, noise, predicted_noise = ddpm_model.predict(x=test_sequences,
                                                                                          batch_size=self.config[
                                                                                              "batch_size"])
        self.assertTrue(predicted_sequences.shape[1:] == (self.config["timesteps"], self.config["sequence_length"],
                                                          self.config["sequence_features"]))
